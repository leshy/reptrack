...
export async function runFFT(
    ...
    const keypointData: [number, [number, number, number]][] = []
    for (const [timestamp, keypoint] of history.keypoints(keypointIndex)) {
        keypointData.push([timestamp, keypoint])
    }
    ...
    for (let i = 0; i < windowSize; i++) {
        input[i * 2] = keypointData[i][1][axisIndex] // Real part = coordinate value
